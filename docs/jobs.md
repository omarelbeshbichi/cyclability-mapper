...
## Bounding Box
```bash
docker compose exec app python -m cmm.jobs.build_network --c oslo --south 59.898941 --west 10.667409 --north 59.954596 --east 10.815038 --chunk 5000 --tout 50 --tol 0.0005
```
...
```bash
docker compose exec app python -m cmm.jobs.refresh_osm_data --c oslo --chunk 5000 --tout 50
```
...
```bash
docker compose exec app python -m cmm.jobs.list_city_metrics
```
