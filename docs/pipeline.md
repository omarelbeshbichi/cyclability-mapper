...
In the following, the pipeline workflow to build up a segment network for a city including associated cyclability metrics is laid out.

# Area Definition

Code associated to this section is stored in `src/cmm/data/ingest/geocoding`.
...
Code associated to this section is stored in overpass files in `src/cmm/data/ingest/overpass_*.py`.
...
Code associated to this section is stored in `src/cmm/metrics/compute_metrics`.
...
An aggregated cyclability metrics for the entirety of the city network is computed performing length-weighted average of the segment-level metrics.

Aggregated uncertainty information of the city for each feature is also computed by using a length-weighted average of `missing_info` multiplied for the relative feature weight. A global city uncertainty parameter is defined as sum of all feature uncertainties. More info in metrics documentation.

# Data Storage

Segments, metrics, and reference areas are stored in a PostGIS database defined by a `network_segments` primary table, a `segment_metrics` table storing metrics data, a `refresh_areas` to store associated Polygons, and a `city_metrics` table to store aggregated city metrics and associated uncertainty. More info in database documentation.